...
export async function generateMetadata(
  ...
  return {
    title: `Informations | ${cateringPackage.data.name}`,
...
async function PackageTiersPage({ params, searchParams }: Request) {
  ...
  const currentTier = cateringPackage.data.tiers.find(
    (tier) => String(tier.id) === searchParams.tier,
  );

  const lowestTier = cateringPackage.data.tiers.length
    ? cateringPackage.data.tiers.reduce((min, current) =>
        current.price < min.price ? current : min,
      )
    : null;

  const highestTier = cateringPackage.data.tiers.length
          ...
          <div className="flex gap-x-3 items-center">
            <figure className="w-[100px] h-[120px] relative flex-none rounded-2xl overflow-hidden">
              <Image
                ...
                src={`${process.env.HOST_API}/${cateringPackage.data.thumbnail}`}
                alt={cateringPackage.data.name}
                ...
                <span className="flex gap-x-2 items-center">
                  <figure className="w-[100px] h-[80px] rounded-2xl overflow-hidden relative">
                    <Image
                      fill
                      className="w-full h-full object-cover object-center"
                      src={`${process.env.HOST_API}/${currentTier.photo}`}
